...
if [[ "$PB_ARCHIVE" == "1" ]]; then
  if [[ -z "$PB_RUN_ID" ]]; then
    PB_RUN_ID="run_$(date +%Y%m%d_%H%M%S)_pb"
  fi
  export PB_RUN_ID
  ARCHIVE_DIR="$ROOT/output_archives/$PB_RUN_ID"
...
if [[ "$PB_ARCHIVE" == "1" ]]; then
  if [[ "$SKIP_PLOTS" != "1" ]]; then
    ...
    "$PYTHON" "$ARCHIVE_PY" plot --profile "$PROFILE" --root "$ROOT" --run-id "$PB_RUN_ID"
  fi
  section "Finalize run archive"
  "$ROOT/scripts/archive_run.sh" "$PB_RUN_ID"
