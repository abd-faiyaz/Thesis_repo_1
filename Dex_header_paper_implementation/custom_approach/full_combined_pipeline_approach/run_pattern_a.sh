...
#   3. Preprocess APKs → per-APK shards (multi-dex sum-pool by default)
#   4. Dex stats histogram (optional telemetry)
#   5. Class balance → artifacts/class_balance.json
#   6. Train CombinedNet (resume-safe)
#   7. Evaluate best checkpoint (ACC, F1, AUC)
#   8. Optional: package portable artifacts tarball
#
# Usage:
#   ./run_pattern_a.sh
