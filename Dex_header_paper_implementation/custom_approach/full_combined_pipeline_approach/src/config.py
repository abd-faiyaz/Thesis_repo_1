...
from src.constants import (
    DEFAULT_COMBINED_INPUT_LEN,
    DEFAULT_LEXICON_SIZE,
...
class PipelineConfig:
    """Resolved configuration for the Pattern A combined pipeline."""
...
def validate_model_dims(cfg: PipelineConfig) -> list[str]:
    """Check model section matches header + BoW layout (Phase 1 sanity)."""
    errors: list[str] = []
    model = cfg.model
    pre = cfg.preprocessing
    lexicon_size = int(pre.get("lexicon_size", DEFAULT_LEXICON_SIZE))
    header_dim = int(model.get("header_dim", DEX_HEADER_FEATURE_DIM))
    bow_dim = int(model.get("bow_dim", lexicon_size + 1))
    expected_combined = header_dim + bow_dim

    if header_dim != DEX_HEADER_FEATURE_DIM:
        errors.append(
            f"model.header_dim must be {DEX_HEADER_FEATURE_DIM}, got {header_dim}"
        )
    if bow_dim != lexicon_size + 1:
        errors.append(f"model.bow_dim must be {lexicon_size + 1}, got {bow_dim}")

    combined_input_len = int(
        model.get("combined_input_len", DEFAULT_COMBINED_INPUT_LEN)
    )
    if combined_input_len != expected_combined:
        errors.append(
            f"model.combined_input_len must be {expected_combined}, "
            f"got {combined_input_len}"
        )

    padded = int(model.get("combined_padded_len", combined_input_len))
    if padded < combined_input_len:
        errors.append(
            f"model.combined_padded_len ({padded}) < combined_input_len "
            f"({combined_input_len})"
        )
    return errors
