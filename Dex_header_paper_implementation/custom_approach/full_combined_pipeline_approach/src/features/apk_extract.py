"""Read all classes*.dex and AndroidManifest.xml from APK archives in memory."""
...
class ApkExtractError(Exception):
    """APK could not be opened or is missing a required entry."""
...
def extract_apk_raw_header(
    ...
    *,
    ...
    """Discover all classes*.dex, parse headers, aggregate into one 104-d vector."""
...
def read_android_manifest(apk_path: Path) -> bytes:
    try:
        with zipfile.ZipFile(apk_path, "r") as zf:
            return _read_zip_entry(zf, MANIFEST_ENTRY)
    except (zipfile.BadZipFile, OSError) as exc:
        raise ApkExtractError(f"Failed to open APK: {apk_path}") from exc
