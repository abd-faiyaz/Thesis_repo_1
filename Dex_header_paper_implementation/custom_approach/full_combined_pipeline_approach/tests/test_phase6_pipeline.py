...
def _setup_artifacts(tmp: Path) -> Path:
    ...
    cfg_data["paths"]["artifacts_bundle"] = str(tmp / "pattern_a_bundle.tar.gz")
    ...
    (tmp / "normalization_header.json").write_text(
        '{"mins": [0], "maxs": [1], "feature_dim": 104, "multidex_mode": "sum"}'
    )
    ...
    def test_compute_balance_and_package(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            subprocess.run(
                [
                    sys.executable,
                    str(ROOT / "scripts" / "compute_class_balance.py"),
                    "--config",
                    str(cfg_path),
                ],
                check=True,
