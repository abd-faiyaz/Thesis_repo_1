"""Unit tests for CombinedPipelineDataset and DataLoaders (synthetic shards)."""
...
from src.data.dataloaders import (
    build_dataloaders_from_manifests,
    build_eval_loader,
    build_train_loader,
)
...
class TestCombinedPipelineDataset(unittest.TestCase):
    def test_len_and_item_shapes(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            manifest = _write_synthetic_manifest(Path(tmp))
            ds = CombinedPipelineDataset.from_manifest(manifest)
            self.assertEqual(len(ds), N_SAMPLES)
            self.assertEqual(ds.combined_dim, DEX_HEADER_FEATURE_DIM + BOW_DIM)
