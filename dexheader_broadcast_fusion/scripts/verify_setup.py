#!/usr/bin/env python3
"""P0 — Verify environment, config, corpus, system_actions, BM1 normalization."""
...
def main() -> int:
    print("Dex header + broadcast fusion — environment check (P0)\n")
    errors: list[str] = []
    ...
    if not actions:
        print(f"ERROR: system_actions.json has no actions")
        return 1

    norm_path = cfg.paths.deployed_mlp_header_bundle / "features" / "normalization_header.json"
    if not norm_path.is_file():
        print(f"ERROR: BM1 normalization_header.json missing: {norm_path}")
        return 1
    ...
    print(f"  system_actions: {len(actions)} ({actions_path.relative_to(ROOT)})")
    print(f"  BM1 normalization: {norm_path}")
