"""Train/validation loops for two-input FusionNet."""
...
def train_one_epoch(
    ...
    *,
    ...
    for batch_h, batch_r, batch_y in pbar:
        batch_h = batch_h.to(device)
        batch_r = batch_r.to(device)
        batch_y = _batch_labels(batch_y, device)

        optimizer.zero_grad(set_to_none=True)
        logits = model(batch_h, batch_r)
...
def validation_epoch(
    ...
    *,
    ...
    for batch_h, batch_r, batch_y in iterator:
        batch_h = batch_h.to(device)
        batch_r = batch_r.to(device)
