...
class PathsConfig:
    ...
    system_actions_file: Path
    deployed_mlp_header_bundle: Path
    deployed_mlp_header_checkpoint: Path
...
class PipelineConfig:
    ...
    def model_id(self) -> str:
        return str(self.raw.get("model_id", "dexheader_broadcast_fusion"))

    @property
    def domain(self) -> str:
        return str(self.raw.get("domain", "dex_header_receiver_actions"))
...
def load_config(config_path: Path | str | None = None) -> PipelineConfig:
    ...
    paths = PathsConfig(
        ...
        export=_resolve_path(
            root,
            paths_raw.get("export", "artifacts/export/dexheader_broadcast_fusion"),
        ),
        ...
        system_actions_file=_resolve_path(
            root,
            paths_raw.get(
                "system_actions_file",
                raw.get("features", {}).get("system_actions_file", "assets/system_actions.json"),
            ),
        ),
        ...
        deployed_mlp_header_checkpoint=_resolve_path(
            root,
            paths_raw.get(
                "deployed_mlp_header_checkpoint",
                "../Dex_header_paper_implementation/only_base1_model/artifacts/checkpoints/latest_checkpoint.pth",
            ),
        ),
    )
...
def dex_settings(cfg: PipelineConfig) -> dict[str, Any]:
    dex = cfg.features.get("dex") or {}
    return {
        "mode": str(dex.get("multidex_mode", "sum")),
        "dex_pattern": str(dex.get("dex_pattern", r"^classes(\d*)\.dex$")),
        "max_dex": int(dex.get("max_dex", 3)),
        "feature_dim": int(dex.get("feature_dim", 104)),
    }
