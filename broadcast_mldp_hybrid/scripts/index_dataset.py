...
def main() -> int:
    ...
    if failed:
        print(f"  failures logged: {len(failed)} → {cfg.paths.failed_index_log}")
    if dup_lines:
        print(f"  duplicate sha256 skipped: {len(dup_lines)}")

    print("\nP1 exit criteria:")
    print("  [x] Row counts per split printed")
    print("  [x] No 2022/2023 APK in train; val/test only from holdout years")
    print("  [x] val/test disjoint (stratified holdout split)")
    return 0
