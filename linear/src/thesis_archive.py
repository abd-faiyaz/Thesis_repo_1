"""Live archive mirroring when LR_RUN_ID is set (see run_linregdroid.sh LR_ARCHIVE=1)."""
...
def after_train(checkpoint: Path, meta: dict) -> None:
    ...
    if not arc.enabled:
        return
    arc.log_training_meta(meta)
    arc.mirror_checkpoint(checkpoint)


def after_eval(*written_paths: Path) -> None:
    arc = get_archive()
    if not arc.enabled:
        return
    for path in written_paths:
        if path.is_file():
            arc.mirror_file(path, f"metrics/{path.name}")
