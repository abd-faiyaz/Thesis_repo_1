...
\begin{document}
	...
	\noindent\resizebox{\colw}{!}{%
		\begin{tikzpicture}[
			...
			\node[phase] (setup) at (0,  0.0) {Environment\\\&~Config};
			\node[phase] (index) at (0, -1.3) {APK Corpus\\Indexing};
			\node[phase] (feat)  at (0, -2.6) {Feature\\Extraction};
			\node[phase] (load)  at (0, -3.9) {Dataset\\Construction};
			\node[phase] (arch)  at (0, -5.2) {Model\\Architecture};
			\node[phase] (train) at (0, -6.5) {Training};
			\node[phase] (eval)  at (0, -7.8) {Evaluation \&\\Calibration};
			\node[phase] (onnx)  at (0, -9.1) {ONNX\\Export};
			\node[phase] (parity)at (0,-10.4) {Export\\Verification};
			...
			%% ── Left-side inputs ────────────────────────────────────────────────
			\node[side] (corpus) at (-2.8, -0.65) {APK Corpus\\$\sim$500\,GB};
			\node[side] (split)  at (-2.8, -3.25) {Train: 2020--21\\Test: 2022--23};
			
			\draw[sarr] (corpus.east) -- ++(0.4,0) |- (index.west);
			\draw[sarr] (split.east)  -- ++(0.4,0) |- (feat.west);
			
			%% ── Middle column — policy, bundle, phone ───────────────────────────
			\node[mid] (policy) at (3.0, -5.8) {
				\textbf{Scan Policy}\\
				tier thresholds\\
				ensemble weights
			};
			
			\node[mid, minimum height=0.65cm] (bundle) at (3.0, -9.1) {
				\textbf{Model Bundle}\\
				ONNX model +\\
				thresholds +\\
				vocabularies
			...
			\node[mid] (phone) at (3.0,-11.5) {
				Deploy to\\Android\\app assets
			};
			
			%% eval → policy: routed orthogonally
			\draw[sarr] (eval.east) -- ++(0.6,0) |- (policy.west);
			
			%% onnx → bundle: horizontal arrow
			\draw[arr] (onnx.east) -- (bundle.west);
			
			%% parity → bundle: safely bypass to bundle bottom
			\draw[sarr] (parity.east) -| ([xshift=-10pt]bundle.south) 
			node[pos=0.75, right, lbl] {verified};
			
			%% bundle → phone
			\draw[arr] (bundle.south) -- (phone.north);
			
			%% policy → phone: strict route far right of bundle to avoid overlay
			\draw[sarr] (policy.east) -- (4.2, -5.8) -- (4.2, -11.5) -- (phone.east)
			node[pos=0.8, right, lbl] {policy};
			
			%% ── Right column — inference tiers ──────────────────────────────────
			\node[tierbox] (tlight) at (6.2, -4.5) {
				\textbf{Light Tier}\\
				ByteCNN, LinReg,\\
				MLDP, Broadcast
			};
			
			\node[tierbox] (tmed) at (6.2, -7.5) {
				\textbf{Medium Tier}\\
				DEX Header MLP,\\
				Pattern A/B
			};
			
			\node[tierbox] (theavy) at (6.2,-10.5) {
				\textbf{Heavy Tier}\\
				XGBoost\\(manifest tokens)
			};
			
			%% bundle → tiers: orthogonal bus routing avoids diagonals entirely
			\coordinate (bus) at (4.8, -9.1);
			\draw[sarr] (bundle.east) -- (bus);
			\draw[sarr] (bus) |- (tlight.west);
			\draw[sarr] (bus) |- (tmed.west);
			\draw[sarr] (bus) |- (theavy.west);
			
			%% policy → tiers: route cleanly between boxes
			\draw[sarr] (policy.north) -- ++(0,0.5) -| (tmed.north)
			node[pos=0.25, above, lbl] {gates};
