...
def load_config(config_path: Path | str | None = None) -> PipelineConfig:
    ...
    paths = PathsConfig(
        ...
        mldp_dir=_resolve_path(root, paths_raw.get("mldp_dir", "artifacts/mldp")),
        transactions_dir=_resolve_path(
            root, paths_raw.get("transactions_dir", "artifacts/transactions")
        ),
        ...
        dataset_index=_resolve_path(
            root, paths_raw.get("dataset_index", "artifacts/dataset_index.csv")
        ),
        ...
        selected_permissions=_resolve_path(
            root,
            paths_raw.get("selected_permissions", "artifacts/mldp/selected_permissions.json"),
        ),
        latest_checkpoint=_resolve_path(
            root,
            paths_raw.get("latest_checkpoint", "artifacts/checkpoints/mldp_pruned.pth"),
        ),
    )
...
def ensure_artifact_dirs(cfg: PipelineConfig) -> None:
    cfg.paths.mldp_dir.mkdir(parents=True, exist_ok=True)
    cfg.paths.transactions_dir.mkdir(parents=True, exist_ok=True)
