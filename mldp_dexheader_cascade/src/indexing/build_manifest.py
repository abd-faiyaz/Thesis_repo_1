"""P1 — Build APK index with BM1 temporal split (val from train years, test = all holdout)."""
...
def scan_apk_rows(
    ...
    *,
    ...
    for row in _index_progress(candidate_rows, desc=dedupe_desc):
        ...
        if digest in seen:
            dup_lines.append(
                f"duplicate_sha256\t{digest}\t{row.apk_path}\tkept={seen[digest].apk_path}"
            )
            continue
        if not row.sha256:
...
def year_label_split_summary(rows: list[IndexRow]) -> list[tuple[int | None, str, int, int, int]]:
    """Return sorted (year, split, benign, malware, total) tuples."""
    counts: Counter[tuple[int | None, str, int]] = Counter()
    for row in rows:
        counts[(row.year, row.split, row.label)] += 1

    keys = sorted({(year, split) for year, split, _ in counts})
    out: list[tuple[int | None, str, int, int, int]] = []
    for year, split in keys:
        benign = counts.get((year, split, 0), 0)
        malware = counts.get((year, split, 1), 0)
        out.append((year, split, benign, malware, benign + malware))
    return out
